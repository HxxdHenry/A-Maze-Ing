...
def main():
    ...
    tie = 0  # tie-breaker to keep heap stable
    ...
    def finish_search(found: bool):
        ...
        # Clear overlays so only path remains
        in_open.clear()
        closed.clear()
        state = "cleaning"
    ...
    while running:
        ...
        # Dijkstra main loop (animated by token bucket)
        if state == "searching":
            ...
            for _ in range(steps):
                if not open_heap:
                    # No path (shouldn't happen in a connected maze)
                    finish_search(False)
                    break
                ...
                # Expand neighbors (uniform cost of 1 per move)
